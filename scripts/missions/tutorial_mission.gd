...
## Туториал в двух локациях (персистентный, переживает переключения).
## Кладбище: собраться из костей → пройтись туда-обратно → рукой открыть ворота →
## встреча с хозяйкой у чёрного входа. Особняк: подмести кухню → посуда →
## завтрак (мука в запертой зельеварочной) → подать наверх в гостиную.
...
func bind_outdoor(g: GraveyardScene, s: Mansion) -> void:
	...
	# знакомство уже было — ведьмы на крыльце больше нет
	if stage > Stage.MEET:
		s.witch.queue_free()
	_enter(stage)
	...
	match s:
		...
		Stage.SWEEP:
			...
			if mansion:
				...
				Game.hint("Веник — в кладовке мётел: 2-й этаж, восточное крыло, за спальнями. Хватай ЛКМ и таскай по пыли на кухне.")
			elif shell:
				Game.hint("Пыль сама себя не подметёт. Заходи в дом (чёрная дверь).")
				_beacon_at(shell.back_portal.global_position)
		...
		Stage.DISHES:
			...
			if _plates_done >= _plates_total:
				_dishes_complete()
		Stage.BREAKFAST:
			Game.objective_changed.emit("Завтрак: яйцо, мука и вино — на плиту, потом E. Мука — в запертой зельеварочной (1-й этаж, восточное крыло).")
			if mansion:
				_beacon_at(mansion.stove.global_position)
				Game.hint("Вентиляция зельеварочной — у пола в коридоре восточного крыла. Рука (F) пролезет и дёрнет рычаг.")
		Stage.SERVE:
		...
		Stage.DONE:
			Game.objective_changed.emit("Рабочий день окончен! Особняк и кладбище — твоя песочница. Скоро — город (M2).")
			_beacon.visible = false
...
func _physics_process(_delta: float) -> void:
	...
	match stage:
		Stage.WALK_OUT:
			if player.global_position.distance_to(graveyard.global_position + graveyard.walk_marker_out) < 1.6:
				_enter(Stage.WALK_BACK)
		Stage.WALK_BACK:
			if player.global_position.distance_to(graveyard.global_position + graveyard.walk_marker_back) < 1.6:
...
func _on_talk() -> void:
	...
	match stage:
		Stage.MEET:
			w.say("О. Ожил. Значит так: особняк — свинарник, у меня депрессия и лапки. Начни с кухни — подмети. Веник где-то наверху.", 7.0)
			w.go_home_puff()
			_enter(Stage.SWEEP)
		Stage.SWEEP:
			w.say("Веник в кладовке наверху. Да, я храню веники в другом крыле от пыли. Логистика.")
		Stage.DISHES:
			w.say("Посуда сама себя не помоет. И не разобьёт. К сожалению.")
		Stage.BREAKFAST:
			w.say("Яйцо, мука, вино. Всё на плиту. Это не рецепт, это стиль жизни.")
		Stage.SERVE:
